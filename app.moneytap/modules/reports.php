...
if ($export_format === 'csv') {
    ...
    switch ($report_type) {
        case 'portfolio':
            ...
            $q = buildPortfolioQuery($conn, $start_date, $end_date, $customer_filter, $status_filter);
            break;
        case 'instalments':
            $title = 'Loan Instalments Report';
            $q = buildInstalmentsQuery($conn, $start_date, $end_date, $customer_filter, $status_filter);
            break;
        case 'customers':
            $title = 'Customers Report';
            $q = buildCustomersQuery($conn, $start_date, $end_date, $customer_filter);
            break;
        case 'overdue':
            $title = 'Overdue Loans Report';
            $q = buildOverdueQuery($conn, $start_date, $end_date, $customer_filter);
            break;
        case 'payments':
            $title = 'Payments Report';
            $q = buildPaymentsQuery($conn, $start_date, $end_date, $customer_filter);
            break;
        case 'provisions':
            $title = 'Provisions Report';
            $q = buildProvisionsQuery($conn, $start_date, $end_date, $customer_filter);
            break;
        case 'summary':
            $title = 'Portfolio Summary Report';
            $q = buildSummaryQuery($conn, $start_date, $end_date, $customer_filter);
            break;
        default:
            $title = 'Report';
            $q = '';
...
function buildInstalmentsQuery($conn, $sd, $ed, $cf, $sf)
{
    ...
    if ($sf && $sf !== 'all') {
        if ($sf === 'paid')
            $w[] = "li.balance_remaining <= 0";
        elseif ($sf === 'unpaid')
            $w[] = "li.balance_remaining > 0";
        elseif ($sf === 'overdue')
            $w[] = "li.balance_remaining > 0 AND li.due_date < CURDATE()";
    }
    $wc = implode(' AND ', $w);
...
function buildProvisionsQuery($conn, $sd, $ed, $cf)
{
    ...
    $w = [
        "(SELECT COUNT(*) FROM loan_instalments WHERE loan_id = lp.loan_id AND balance_remaining > 0 AND due_date < CURDATE()) > 0",
...
function buildSummaryQuery($conn, $sd, $ed, $cf)
{
    ...
    if ($cf)
        $date_clause .= " AND lp.customer_id = " . intval($cf);

    $penalty_clause = "lp2.disbursement_date BETWEEN '{$sd}' AND '{$ed}'";
    if ($cf)
        $penalty_clause .= " AND lp2.customer_id = " . intval($cf);
...
function generateCsv($data, $report_type, $title, $sd, $ed)
{
    ...
    // ── Meta header block ────────────────────────────────────────────────────
    fputcsv($out, ['MoneyTap Ecosystem']);
    fputcsv($out, ['Kigali, Rwanda | Tel: +250 000 000 000 | Email: hello@moneytap.rw']);
        ...
        case 'payments':
            ...
            foreach ($data as $r) {
                ...
                $rows[] = [
                    $r['loan_number'],
                    $r['customer_name'],
                    ($r['instalment_number'] > 0 ? $r['instalment_number'] : 'DISB'),
                    $r['due_date'] ? date('Y-m-d', strtotime($r['due_date'])) : '',
...
if ($customer_filter)
    $stats_sql .= " AND lp.customer_id = " . intval($customer_filter);
...
<body>
    <div class="container-fluid py-4">
        ...
        <div class="card mb-4">
                    ...
                    <div class="row g-3">
                        <div class="col-md-3">
                            ...
                            <select name="report_type" id="reportTypeSelect" class="form-select" required>
                                <option value="portfolio" <?= $report_type == 'portfolio' ? 'selected' : '' ?>>Loan Portfolio
                                </option>
                                <option value="instalments" <?= $report_type == 'instalments' ? 'selected' : '' ?>>Loan
                                    Instalments</option>
                                <option value="customers" <?= $report_type == 'customers' ? 'selected' : '' ?>>Customers
                                </option>
                                <option value="overdue" <?= $report_type == 'overdue' ? 'selected' : '' ?>>Overdue Loans
                                </option>
                                <option value="payments" <?= $report_type == 'payments' ? 'selected' : '' ?>>Payments</option>
                                <option value="provisions" <?= $report_type == 'provisions' ? 'selected' : '' ?>>Provisions
                                </option>
                                <option value="summary" <?= $report_type == 'summary' ? 'selected' : '' ?>>Portfolio Summary
                                </option>
                            </select>
                        </div>
                        <div class="col-md-2">
                            ...
                            <input type="date" class="form-control" name="start_date" id="startDate"
                                onchange="validateAndTrigger()" value="<?= htmlspecialchars($start_date) ?>" required>
                        </div>
                        <div class="col-md-2">
                            <label class="form-label fw-semibold">End Date</label>
                            <input type="date" class="form-control" name="end_date" id="endDate"
                                onchange="validateAndTrigger()" value="<?= htmlspecialchars($end_date) ?>" required>
                        </div>
                        ...
                        <div class="col-md-2">
                            ...
                            <select name="status_filter" class="form-select">
                                ...
                                <option value="Suspended" <?= $status_filter == 'Suspended' ? 'selected' : '' ?>>Suspended
                                </option>
                                <option value="Closed" <?= $status_filter == 'Closed' ? 'selected' : '' ?>>Closed</option>
                                <option value="paid" <?= $status_filter == 'paid' ? 'selected' : '' ?>>Paid (Instalments)
                                </option>
                                <option value="unpaid" <?= $status_filter == 'unpaid' ? 'selected' : '' ?>>Unpaid
                                    (Instalments)</option>
                            </select>
                        </div>
                    </div>
            ...
            <div class="col-md-3">
                ...
                <div class="input-group">
                    <input type="month" id="selectedMonth" class="form-control" onchange="applyMonthMacro(this.value)"
        ...
        <div class="d-flex flex-wrap align-items-center gap-2">
            ...
            <button type="button" class="btn btn-sm btn-outline-secondary" onclick="setRange('month')">This
                Month</button>
            <button type="button" class="btn btn-sm btn-outline-secondary" onclick="setRange('quarter')">This
                Quarter</button>
            <button type="button" class="btn btn-sm btn-outline-secondary" onclick="setRange('year')">This Year</button>
            <button type="button" class="btn btn-sm btn-outline-secondary" onclick="setRange('last_month')">Last
                Month</button>
            <button type="button" class="btn btn-sm btn-outline-secondary" onclick="setRange('last_quarter')">Last
                Quarter</button>
            <button type="button" class="btn btn-sm btn-outline-secondary" onclick="setRange('last_year')">Last
                Year</button>
        </div>
    </div>
    </div>
